...
// C'est comme importer des jouets dans ta chambre pour jouer !// 📦 On importe les outils de Bevy pour faire notre jeu
...
fn setup_game(
    ...
    // 🎨 Style de texte commun pour tous les textes du bandeau
    let text_style = TextStyle {
        font_size: 30.0,                           // Taille de la police
        color: Color::srgb(1.0, 1.0, 1.0),        // Couleur blanche
        ..default()
    };
    
    // 🏆 SCORE - À gauche du bandeau
    commands.spawn((
        ScoreText,  // Étiquette pour retrouver ce texte plus tard
        Text2dBundle {
            text: Text::from_section(
                "Score: 0",      // Texte initial
                text_style.clone(), // Le style défini plus haut
            ),
            transform: Transform::from_xyz(-350.0, 340.0, 1.0), // Position en haut à gauche
            ..default()
        },
    ));
    
    // 🌊 NIVEAU - Au centre du bandeau
    commands.spawn((
        LevelText,  // Étiquette pour retrouver ce texte plus tard
        Text2dBundle {
            text: Text::from_section(
                "Vague: 1",          // Texte initial
                text_style.clone(),  // Le même style
            ),
            transform: Transform::from_xyz(0.0, 340.0, 1.0), // Position en haut au centre
            ..default()
        },
    ));
    
    // ❤️ VIES - À droite du bandeau
    commands.spawn((
        LivesText,  // Étiquette pour retrouver ce texte plus tard
        Text2dBundle {
            text: Text::from_section(
                "Vies: 3",           // Texte initial
                text_style,          // Le même style
            ),
            transform: Transform::from_xyz(350.0, 340.0, 1.0), // Position en haut à droite
            ..default()
        },
    ));
}
...
fn display_info(
    ...
    if let Ok(mut text) = level_text_query.get_single_mut() {
        ...
        text.sections[0].value = format!("Vague: {}", wave_manager.current_wave);
    }
    
    // ❤️ METTRE À JOUR LES VIES
    // On cherche le texte des vies et on le modifie
    if let Ok(mut text) = lives_text_query.get_single_mut() {
        // S'il y a un joueur, on affiche ses vies
        if let Some(health) = player_query.iter().next() {
            text.sections[0].value = format!("Vies: {}", health.current);
        } else {
            // Si le joueur est mort, on affiche 0
            text.sections[0].value = "Vies: 0".to_string();
        }
    }
}
