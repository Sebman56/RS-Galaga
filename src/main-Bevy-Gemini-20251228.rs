// ═══════════════════════════════════════════════════════════════════════════
// 🎮 Code source en Rust du jeu Xgalaga selon Gemini AI le 2025-12-28
// ═══════════════════════════════════════════════════════════════════════════
...
fn player_shooting(mut commands: Commands, asset_server: Res<AssetServer>, keyboard: Res<ButtonInput<KeyCode>>, query: Query<&Transform, With<Player>>, state: Res<GameState>) {
    if state.game_over { return; }
...
fn wave_spawner(
    ...
    // CORRECTION : On déballe le Result de la fenêtre ici
...
fn despawn_out_of_bounds(mut commands: Commands, query: Query<(Entity, &Transform, Option<&Bullet>, Option<&Enemy>)>, window_query: Query<&Window, With<PrimaryWindow>>) {
    let window = window_query.single().expect("Pas de fenêtre");
    let h = window.height() / 2.0 + 50.0;
    let w = window.width() / 2.0 + 50.0;
    for (e, t, is_b, is_en) in query.iter() {
        let p = t.translation;
        if is_b.is_some() && p.y > h { commands.entity(e).despawn(); }
        if is_en.is_some() && (p.y < -h || p.y > h || p.x < -w || p.x > w) { commands.entity(e).despawn(); }
    }
}

fn bullet_enemy_collision(mut commands: Commands, asset_server: Res<AssetServer>, b_query: Query<(Entity, &Transform), With<Bullet>>, e_query: Query<(Entity, &Transform), With<Enemy>>, mut state: ResMut<GameState>) {
    for (b_e, b_t) in b_query.iter() {
        let b_pos = b_t.translation.xy();
        for (e_e, e_t) in e_query.iter() {
            let e_pos = e_t.translation.xy();
            if (b_pos.x - e_pos.x).abs() < (BULLET_SIZE.x + ENEMY_SIZE.x) / 2.0 && (b_pos.y - e_pos.y).abs() < (BULLET_SIZE.y + ENEMY_SIZE.y) / 2.0 {
...
fn player_enemy_collision(mut commands: Commands, asset_server: Res<AssetServer>, mut p_query: Query<(Entity, &Transform, &mut Health), With<Player>>, e_query: Query<(Entity, &Transform), With<Enemy>>, mut state: ResMut<GameState>) {
    if state.game_over { return; }
    for (p_e, p_t, mut h) in p_query.iter_mut() {
        let p_pos = p_t.translation.xy();
        for (e_e, e_t) in e_query.iter() {
            let e_pos = e_t.translation.xy();
            if (p_pos.x - e_pos.x).abs() < (PLAYER_SIZE.x + ENEMY_SIZE.x) / 2.0 && (p_pos.y - e_pos.y).abs() < (PLAYER_SIZE.y + ENEMY_SIZE.y) / 2.0 {
                spawn_explosion(&mut commands, &asset_server, e_t.translation, ENEMY_SIZE * 1.5);
                commands.entity(e_e).despawn();
                h.current -= 1;
                if h.current <= 0 {
...
fn display_info(wave: Res<WaveManager>, state: Res<GameState>, p_query: Query<&Health, With<Player>>, mut s_text: Query<&mut Text, With<ScoreText>>, mut l_text: Query<&mut Text, (With<LevelText>, Without<ScoreText>, Without<LivesText>)>, mut liv_text: Query<&mut Text, (With<LivesText>, Without<ScoreText>, Without<LevelText>)>) {
    ...
    if let Ok(mut t) = liv_text.single_mut() {
        if let Some(h) = p_query.iter().next() { **t = format!("Vies: {}", h.current); } else { **t = "Vies: 0".to_string(); }
    }
}
...
fn main() {
    App::new()
        ...
        .add_systems(Update, (player_input, player_shooting, wave_spawner))
        .add_systems(Update, (apply_movement, clamp_player_position).chain())
        .add_systems(Update, (despawn_out_of_bounds, bullet_enemy_collision, player_enemy_collision, cleanup_explosions, display_info))
        .run();
}